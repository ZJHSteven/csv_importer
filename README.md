...
- 导入时自动补充章节标签与题型标签（章节标签会拆分牌堆各层级并去重追加）
...
6. 如需限制牌组范围，先勾选“限制牌组范围”，再在树状牌组列表中选择目标牌组；只点选树节点但不勾选限制开关，不会限制扫描范围。
...
- TTS 扫描会区分“需合成”“可复用已有媒体”“已有音频标记跳过”“空文本/字段异常/缺失笔记”，因此界面数量可能和 Anki 牌组里的卡片数不同；Anki 里一个笔记也可能生成多张卡片。
- 导入会话记录保存在 `user_files/import_sessions`，可用于回滚或重复项二次更新。
- 覆盖已生成音频时会重写媒体并更新标记；语速支持填写倍率（如 0.8/1.0/1.2）。
- “仅处理最近导入的笔记”勾选时，如果最近导入范围为空但已经选择牌组，TTS 会自动按选中牌组扫描；如果二者都为空，界面会提示用户调整范围。
- 运行日志只写入 `user_files/logs/csv_importer.log`，不会输出到 Anki 捕获的标准输出/错误流，避免普通 INFO 日志触发“插件遇到问题”的弹窗。
