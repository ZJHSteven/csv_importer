...
## 最近更新
- 修复 Azure TTS 的 SSML 文本规整：对字段内容先做 HTML 实体解码、HTML 标签清理、XML 转义，并在发请求前增加本地 SSML 校验，避免 `&nbsp;` 等内容触发 HTTP 400。
- 新增题型映射：黑体词默写 → 问答题（输入答案，带解释）。
- TTS 页新增“扫描标签”输入框，允许手动指定/留空扫描标签，并用于任务筛选。

- 调整标签合并规则：不再给行内标签自动套牌堆前缀，只在缺少“牌堆路径标签”时补齐章节标签。

- 调整章节标签追加策略：若行内已包含牌堆标签或其子标签，则不再重复追加章节标签。

- 更新标签规则：章节标签改为单一树状标签、题型标签统一使用“题型::”前缀、行内标签自动补齐牌堆前缀并避免重复嵌套。
...
- TTS 进度改为非阻塞对话框，支持隐藏窗口与中止请求。

- 优化 TTS 扫描诊断：区分候选、需合成、可复用、已有标记跳过、空文本与异常，并将扫描摘要和 HTTP 失败详情写入本地日志。

- 修复 TTS 日志弹窗问题：移除标准输出日志，仅写文件日志；最近导入范围为空时按选中牌组回退扫描。
